...
| Endpoint | Solscan label | Approx. SOL received | Branch |
|---|---|---:|---|
| `3AfnRwXvWxu4HpA6HQQwMzWfP6bETq62oUrwPMfHJ2rH` | **CCE.Cash: Exchange Deposit Wallet** | ~1,337 | `CoSKZDV8` + `HLSU45P2` + `HHRZoUMx` |
| `eS4n56zrQ4ESznC8mDxQhsY4JoCpEt1jDczgcQ299qW` | **#Kucoin Exchange · #Deposit Address** | ~544 | `7M79fHZ8` + `AZN1ecqe` + `GokFVhEr` |
| `BmFdpraQhkiDQE6SnfG5omcA1VwzqfXrwtNYBwWTymy6` | **KuCoin Hot Wallet (BmFdp)** | ~434 | via `GPscfRmNg` |
| `EceGdm2R84yGqPHcQ6BR1qU994tM2MUkA2gAXQjoeyFZ` | (fresh relay, still in transit) | ~391 | via `btYkifQc` |
...
- **CCE.Cash:** ~1,337 SOL
- **KuCoin:** ~978 SOL (deposit address `eS4n…` ~544 + hot wallet `BmFdp…` ~434)
- **Still in transit** through fresh relays (branch via `EceGdm…`): ~391 SOL
- Remainder in dust / micro-branches.

*(A naive automated tally over-counts because several intermediate wallets recycle funds along shared branches; the figures above are computed from the labeled terminals, not from that tally.)*
